...
int BMI160::init()
{
	...
	_accel_topic = orb_advertise_multi(ORB_ID(sensor_accel), &arp, &_accel_orb_class_instance, (external()) ? ORB_PRIO_MAX - 1 : ORB_PRIO_HIGH - 1);
	...
	_gyro->_gyro_topic = orb_advertise_multi(ORB_ID(sensor_gyro), &grp, &_gyro->_gyro_orb_class_instance, (external()) ? ORB_PRIO_MAX - 1 : ORB_PRIO_HIGH - 1);
...
void BMI160::_set_dlpf_filter(uint16_t bandwidth)
{
	_dlpf_freq = 0;
...
ssize_t BMI160::read(struct file *filp, char *buffer, size_t buflen)
{
...
ssize_t BMI160::gyro_read(struct file *filp, char *buffer, size_t buflen)
{
...
int BMI160::ioctl(struct file *filp, int cmd, unsigned long arg)
{
	switch (cmd)
	{
...
int BMI160::gyro_ioctl(struct file *filp, int cmd, unsigned long arg)
{
	switch (cmd)
	{
...
void BMI160::write_reg(unsigned reg, uint8_t value)
{
	uint8_t cmd[2];
...
void BMI160::modify_reg(unsigned reg, uint8_t clearbits, uint8_t setbits)
{
	uint8_t val;
...
void BMI160::write_checked_reg(unsigned reg, uint8_t value)
{
	write_reg(reg, value);
...
void BMI160::start()
{
	...
	hrt_call_every(&_call, 1000, _call_interval - BMI160_TIMER_REDUCTION, (hrt_callout) &BMI160::measure_trampoline, this);
	reset();
}
...
void BMI160::measure_trampoline(void *arg)
{
