...
int BMI055_accel::accel_set_sample_rate(float frequency)
{
	uint8_t setbits = 0;
...
ssize_t BMI055_accel::read(struct file *filp, char *buffer, size_t buflen)
{
...
int BMI055_accel::accel_self_test()
{
	if (self_test())
	{
		return 1;
...
int BMI055_accel::ioctl(struct file *filp, int cmd, unsigned long arg)
{
	switch (cmd)
	{
...
void BMI055_accel::start()
{
	...
	hrt_call_every(&_call, 1000, _call_interval - BMI055_TIMER_REDUCTION, (hrt_callout) &BMI055_accel::measure_trampoline, this);
	reset();
}
...
void BMI055_accel::measure_trampoline(void *arg)
{
...
void BMI055_accel::measure()
{
	...
	if (report.accel_x == 0 && report.accel_y == 0 && report.accel_z == 0 && report.temp == 0)
	{
