...
void stop(bool external_bus, enum sensor_type sensor)
{
	BMI055_accel **g_dev_acc_ptr = external_bus ? &g_acc_dev_ext : &g_acc_dev_int;
	BMI055_gyro **g_dev_gyr_ptr = external_bus ? &g_gyr_dev_ext : &g_gyr_dev_int;
 ...
 * and automatic modes.
 */
void test(bool external_bus, enum sensor_type sensor)
{
	const char *path_accel = external_bus ? BMI055_DEVICE_PATH_ACCEL_EXT : BMI055_DEVICE_PATH_ACCEL;
	const char *path_gyro = external_bus ? BMI055_DEVICE_PATH_GYRO_EXT : BMI055_DEVICE_PATH_GYRO;
	...
	if (sensor == BMI055_ACCEL)
	{
		...
		if (fd_acc < 0)
			err(1, "%s Accel file open failed (try 'bmi055 -A start')", path_accel);
		...
		warnx("acc range: %8.4f m/s^2 (%8.4f g)", (double )a_report.range_m_s2, (double)(a_report.range_m_s2 / BMI055_ONE_G));
	...
	if (sensor == BMI055_GYRO)
	{
		...
		warnx("gyr range: %8.4f rad/s (%d deg/s)", (double )g_report.range_rad_s, (int )((g_report.range_rad_s / M_PI_F) * 180.0f + 0.5f));
 ...
 * Reset the driver.
 */
void reset(bool external_bus, enum sensor_type sensor)
{
	const char *path_accel = external_bus ? BMI055_DEVICE_PATH_ACCEL_EXT : BMI055_DEVICE_PATH_ACCEL;
	const char *path_gyro = external_bus ? BMI055_DEVICE_PATH_GYRO_EXT : BMI055_DEVICE_PATH_GYRO;
...
void BMI055::write_reg(unsigned reg, uint8_t value)
{
	uint8_t cmd[2];
