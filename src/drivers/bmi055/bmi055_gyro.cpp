...
int BMI055_gyro::init()
{
	...
	_gyro_topic = orb_advertise_multi(ORB_ID(sensor_gyro), &grp, &_gyro_orb_class_instance, (external()) ? ORB_PRIO_MAX - 1 : ORB_PRIO_HIGH - 1);
...
int BMI055_gyro::gyro_self_test()
{
	if (self_test())
	{
		return 1;
	...
	if ((fabsf(_gyro_scale.x_offset) < 0.000001f) && (fabsf(_gyro_scale.y_offset) < 0.000001f) && (fabsf(_gyro_scale.z_offset) < 0.000001f))
	{
...
ssize_t BMI055_gyro::read(struct file *filp, char *buffer, size_t buflen)
{
...
int BMI055_gyro::ioctl(struct file *filp, int cmd, unsigned long arg)
{
	switch (cmd)
	{
...
void BMI055_gyro::start()
{
	...
	hrt_call_every(&_call, 1000, _call_interval - BMI055_TIMER_REDUCTION, (hrt_callout) &BMI055_gyro::measure_trampoline, this);
	reset();
}
...
void BMI055_gyro::measure_trampoline(void *arg)
{
...
void BMI055_gyro::measure()
{
	...
	if (report.temp == 0 && report.gyro_x == 0 && report.gyro_y == 0 && report.gyro_z == 0)
	{
