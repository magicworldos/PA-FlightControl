...
__EXPORT const char *__px4_log_level_color[_PX4_LOG_LEVEL_PANIC + 1] = { PX4_ANSI_COLOR_RESET, PX4_ANSI_COLOR_GREEN, PX4_ANSI_COLOR_YELLOW, PX4_ANSI_COLOR_RED, PX4_ANSI_COLOR_RED };
...
__EXPORT void px4_log_modulename(int level, const char *moduleName, const char *fmt, ...)
{
	...
	if (level >= _PX4_LOG_LEVEL_WARN && orb_log_message_pub)   //only publish important messages
	{
		...
		const uint8_t log_level_table[] = { 6, /* _PX4_LOG_LEVEL_ALWAYS */
