...
static int getopt_internal(int argc, char **argv, const char *shortopts, const GETOPT_LONG_OPTION_T *longopts, int *longind, int only)
{
	...
	if (argc == 0 || argv == NULL || (shortopts == NULL && longopts == NULL) || optind >= argc || argv[optind] == NULL)
	{
		return EOF;
	}
	...
	if (longopts != NULL && (memcmp(argv[optind], "--", 2) == 0 || (only && argv[optind][0] == '+')) && optwhere == 1)
	{
		/* handle long options */
		...
		{
			if (memcmp(argv[optind] + optwhere, longopts[optindex].name, match_chars) == 0)
			{
