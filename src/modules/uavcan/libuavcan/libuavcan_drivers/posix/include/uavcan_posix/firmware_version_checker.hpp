...
class FirmwareVersionChecker: public uavcan::IFirmwareVersionChecker
{
	...
	enum
	{
		MaxPathLength = uavcan::protocol::file::Path::FieldTypes::path::MaxSize + MaxBasePathLength
	};
...
protected:
	...
	virtual bool shouldRequestFirmwareUpdate(uavcan::NodeID, const uavcan::protocol::GetNodeInfo::Response& node_info, FirmwareFilePath& out_firmware_file_path)
	{
		...
		int n = snprintf(fname_root, sizeof(fname_root), "%s%s/%d.%d", getFirmwareBasePath().c_str(), node_info.name.c_str(), node_info.hardware_version.major, node_info.hardware_version.minor);
...
public:
	...
	const BasePathString& getFirmwareCachePath() const
	{
		return cache_path_;
	}
	...
	int createFwPaths(const char* base_path)
	{
		...
		if (base_path)
		{
			...
			if (len > 0 && len < base_path_.MaxSize)
			{
				...
				if (rv >= 0)
				{
					if ((getFirmwareCachePath().size() + uavcan::protocol::file::Path::FieldTypes::path::MaxSize) > MaxPathLength)
					{
