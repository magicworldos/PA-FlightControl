...
void init()
{
	...
	do
	{
		...
		res = can.init([]()
		{	::usleep(can.getRecommendedListeningDelay().toUSec());}, bitrate);
		if (res >= 0)
		{
...
class : public chibios_rt::BaseStaticThread<8192>
{
	...
	void performDynamicNodeIDAllocation()
	{
		...
		lowsyslog("Dynamic node ID %d allocated by %d\n", int(client.getAllocatedNodeID().get()), int(client.getAllocatorNodeID().get()));
...
public:
	msg_t main()
	{
		...
		while (true)
		{
			...
			lowsyslog("Memory usage: free=%u used=%u worst=%u\n", getNode().getAllocator().getNumFreeBlocks(), getNode().getAllocator().getNumUsedBlocks(), getNode().getAllocator().getPeakNumUsedBlocks());
			...
			node.logInfo("app", "UTC %* sec, %* corr, %* jumps", uavcan_stm32::clock::getUtc().toMSec() / 1000, uavcan_stm32::clock::getUtcSpeedCorrectionPPM(), uavcan_stm32::clock::getUtcAjdustmentJumpCount());
#endif
		}
		return msg_t();
	}
