...
public:
	...
	std::uint32_t getOverflowCount() const
	{
...
BitTimingSettings computeBitTimings(std::uint32_t bitrate)
{
	if (Chip_Clock_GetSystemClockRate() == 48000000) // 48 MHz is optimal for CAN timings
	{
		...
		{
			case 1000000:
				return BitTimingSettings { 0, 0x0505 }; // 8  quanta, 87.5%
			case 500000:
				return BitTimingSettings { 0, 0x1c05 }; // 16 quanta, 87.5%
			case 250000:
				return BitTimingSettings { 0, 0x1c0b }; // 16 quanta, 87.5%
			case 125000:
				return BitTimingSettings { 0, 0x1c17 }; // 16 quanta, 87.5%
			case 100000:
				return BitTimingSettings { 0, 0x1c1d }; // 16 quanta, 87.5%
			default:
...
{
	static constexpr uavcan::uint32_t BitRatesToTry[] = { 1000000, 500000, 250000, 125000, 100000 };
...
{
	{
		...
		static CCAN_CALLBACKS_T ccan_callbacks = { canRxCallback, canTxCallback, canErrorCallback, nullptr, nullptr, nullptr, nullptr, nullptr };
...
uavcan::int16_t CanDriver::send(const uavcan::CanFrame& frame, uavcan::MonotonicTime tx_deadline, uavcan::CanIOFlags flags)
{
	if (frame.isErrorFrame() || frame.dlc > 8 || (flags & uavcan::CanIOFlagLoopback) != 0)   // TX timestamping is not supported by this driver.
	{
		return -1;
	}
...
uavcan::int16_t CanDriver::receive(uavcan::CanFrame& out_frame, uavcan::MonotonicTime& out_ts_monotonic, uavcan::UtcTime& out_ts_utc, uavcan::CanIOFlags& out_flags)
{
...
uavcan::int16_t CanDriver::select(uavcan::CanSelectMasks& inout_masks, const uavcan::CanFrame* (&)[uavcan::MaxCanIfaces], uavcan::MonotonicTime blocking_deadline)
{
	...
	const bool noblock = ((inout_masks.read == 1) && hasReadyRx()) || ((inout_masks.write == 1) && hasEmptyTx());
...
uavcan::int16_t CanDriver::configureFilters(const uavcan::CanFilterConfig* filter_configs, uavcan::uint16_t num_configs)
{
	CriticalSectionLocker locker;
