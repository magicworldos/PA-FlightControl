...
uavcan_linux::NodePtr initNodeWithDynamicID(const std::vector<std::string>& ifaces, const std::uint8_t instance_id, const uavcan::NodeID preferred_node_id, const std::string& name)
{
	/*
	...
	std::cout << "Node ID " << int(client.getAllocatedNodeID().get()) << " allocated by " << int(client.getAllocatorNodeID().get()) << std::endl;
...
int main(int argc, const char** argv)
{
	try
	{
		...
		uavcan_linux::NodePtr node = initNodeWithDynamicID(std::vector<std::string>(argv + 2, argv + argc), std::uint8_t(instance_id), std::uint8_t(instance_id), "org.uavcan.linux_test_dynamic_node_id_client");
		runForever(node);
