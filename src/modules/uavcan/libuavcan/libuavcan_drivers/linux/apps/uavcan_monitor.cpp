...
class Monitor: public uavcan::NodeStatusMonitor
{
	...
	{
		static const std::unordered_map<std::uint8_t, std::pair<CLIColor, std::string>> map { { uavcan::protocol::NodeStatus::HEALTH_OK, { CLIColor(CLIColor::Green), "OK" } }, { uavcan::protocol::NodeStatus::HEALTH_WARNING, { CLIColor(CLIColor::Yellow), "WARNING" } }, { uavcan::protocol::NodeStatus::HEALTH_ERROR, { CLIColor(CLIColor::Magenta), "ERROR" } }, { uavcan::protocol::NodeStatus::HEALTH_CRITICAL, { CLIColor(CLIColor::Red), "CRITICAL" } } };
		try
		{
		...
		{
			return
			{	CLIColor(CLIColor::Red), std::to_string(health)};
		}
	}
	
	static std::pair<CLIColor, std::string> modeToColoredString(const std::uint8_t mode)
	{
		static const std::unordered_map<std::uint8_t, std::pair<CLIColor, std::string>> map { { uavcan::protocol::NodeStatus::MODE_OPERATIONAL, { CLIColor(CLIColor::Green), "OPERATIONAL" } }, { uavcan::protocol::NodeStatus::MODE_INITIALIZATION, { CLIColor(CLIColor::Yellow), "INITIALIZATION" } }, { uavcan::protocol::NodeStatus::MODE_MAINTENANCE, { CLIColor(CLIColor::Cyan), "MAINTENANCE" } }, { uavcan::protocol::NodeStatus::MODE_SOFTWARE_UPDATE, { CLIColor(CLIColor::Magenta), "SOFTWARE_UPDATE" } }, { uavcan::protocol::NodeStatus::MODE_OFFLINE, { CLIColor(CLIColor::Red), "OFFLINE" } } };
		try
		{
			return map.at(mode);
		}
		...
		{
			return
			{	CLIColor(CLIColor::Red), std::to_string(mode)};
		}
	}
	...
	void printStatusLine(const uavcan::NodeID nid, const uavcan::NodeStatusMonitor::NodeStatus& status)
	{
		...
		std::printf("| %-10lu | %04x  %s'%s  %u\n", uptime, vendor_code, std::bitset<8>((vendor_code >> 8) & 0xFF).to_string().c_str(), std::bitset<8>(vendor_code).to_string().c_str(), vendor_code);
	}
...
{
	auto node = uavcan_linux::makeNode(ifaces, node_name.c_str(), uavcan::protocol::SoftwareVersion(), uavcan::protocol::HardwareVersion());
