...
public:
	...
	void flushTxQueueTo(uavcan::INode& main_node, std::uint8_t iface_index)
	{
		...
		{
			UAVCAN_TRACE("VirtualCanIface", "TX injection [iface=0x%02x]: %s", unsigned(iface_mask), e->toString().c_str());
			
			const int res = main_node.injectTxFrame(e->frame, e->deadline, iface_mask, uavcan::CanTxQueue::Qos(e->qos), e->flags);
...
class VirtualCanDriver: public uavcan::ICanDriver, public uavcan::IRxFrameListener, public ITxQueueInjector, uavcan::Noncopyable
{
	class Event
	{
		std::mutex m_;
...
public:
			    ...
			    num_ifaces_(arg_num_ifaces)
	{
		...
		UAVCAN_TRACE("VirtualCanDriver", "Total blocks: %u, quota per queue: %u", unsigned(allocator_.getBlockCapacity()), unsigned(quota_per_queue));
		
		for (unsigned i = 0; i < num_ifaces_; i++)
		{
			ifaces_[i].template construct<uavcan::IPoolAllocator&, uavcan::ISystemClock&, std::mutex&, unsigned>(allocator_, clock_, mutex_, quota_per_queue);
		}
	}
};
...
static void runSubNode(const uavcan_linux::SubNodePtr& node)
{
	 ...
	 * Log subscriber
	 */
	auto log_sub = node->makeSubscriber<uavcan::protocol::debug::LogMessage>([](const uavcan::ReceivedDataStructure<uavcan::protocol::debug::LogMessage>& msg)
	{	
	...
	struct NodeStatusMonitor: public uavcan::NodeStatusMonitor
	{
		...
		{
			std::cout << "Remote node NID " << int(event.node_id.get()) << " changed status: " << event.old_status.toString() << " --> " << event.status.toString() << std::endl;
		}
	};
	 ...
	 * KV subscriber
	 */
	auto kv_sub = node->makeSubscriber<uavcan::protocol::debug::KeyValue>([](const uavcan::ReceivedDataStructure<uavcan::protocol::debug::KeyValue>& msg)
	{	
		std::cout << msg << std::endl;
	});
