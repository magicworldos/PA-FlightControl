...
{
	auto node = uavcan_linux::makeNode(ifaces, name.c_str(), uavcan::protocol::SoftwareVersion(), uavcan::protocol::HardwareVersion(), nid);
	node->setModeOperational();
	return node;
}

template<typename DataType>
typename DataType::Response call(uavcan_linux::BlockingServiceClient<DataType>& client, uavcan::NodeID server_node_id, const typename DataType::Request& request)
{
 ...
 */
const std::map<std::string, std::pair<std::string, std::function<void(const uavcan_linux::NodePtr&, const uavcan::NodeID, const std::vector<std::string>&)> > > commands = { { "param", { "No arguments supplied - requests all params from a remote node\n"
		"<param_name> <param_value> - assigns parameter <param_name> to value <param_value>", [](const uavcan_linux::NodePtr& node, const uavcan::NodeID node_id, const std::vector<std::string>& args)
{	
...
} } }, { "param_save", { "Calls uavcan.protocol.param.ExecuteOpcode on a remote node with OPCODE_SAVE", [](const uavcan_linux::NodePtr& node, const uavcan::NodeID node_id, const std::vector<std::string>&)
{	
	...
	std::cout << call(*client, node_id, request) << std::endl;
} } }, { "param_erase", { "Calls uavcan.protocol.param.ExecuteOpcode on a remote node with OPCODE_ERASE", [](const uavcan_linux::NodePtr& node, const uavcan::NodeID node_id, const std::vector<std::string>&)
{	
	auto client = node->makeBlockingServiceClient<uavcan::protocol::param::ExecuteOpcode>();
	uavcan::protocol::param::ExecuteOpcode::Request request;
	request.opcode = request.OPCODE_ERASE;
	std::cout << call(*client, node_id, request) << std::endl;
} } }, { "restart", { "Restarts a remote node using uavcan.protocol.RestartNode", [](const uavcan_linux::NodePtr& node, const uavcan::NodeID node_id, const std::vector<std::string>&)
{	
...
} } }, { "info", { "Calls uavcan.protocol.GetNodeInfo on a remote node", [](const uavcan_linux::NodePtr& node, const uavcan::NodeID node_id, const std::vector<std::string>&)
{	
...
} } }, { "transport_stats", { "Calls uavcan.protocol.GetTransportStats on a remote node", [](const uavcan_linux::NodePtr& node, const uavcan::NodeID node_id, const std::vector<std::string>&)
{	
...
} } }, { "hardpoint", { "Publishes uavcan.equipment.hardpoint.Command\n"
		"Expected argument: command", [](const uavcan_linux::NodePtr& node, const uavcan::NodeID, const std::vector<std::string>& args)
{	
