...
public:
	struct RecentEvent
	{
		...
		uavcan::MakeString<81>::Type toString() const   // Heapless return
		{
			...
			(void) std::snprintf(reinterpret_cast<char*>(out.begin()), out.size() - 1U, "%-11s  %-28s %-20lld %016llx", timebuf, getEventName(code), static_cast<long long>(argument), static_cast<long long>(argument));
			return out;
		}
...
public:
	...
	const RecentEvent& getEventByIndex(unsigned index) const
	{
	...
	const decltype(event_counters_)& getEventCounters() const
	{
...
std::vector<std::pair<uavcan::dynamic_node_id_server::TraceCode, EventTracer::EventStatisticsRecord>> collectRelevantEvents(const EventTracer& event_tracer, const unsigned num_events)
{
...
CLIColor getColorHash(unsigned value)
{
	return CLIColor(31 + value % 7);
}
...
void redraw(const uavcan_linux::NodePtr& node, const uavcan::MonotonicTime timestamp, const EventTracer& event_tracer, const uavcan::dynamic_node_id_server::DistributedServer& server)
{
	...
	render_top_int("Commit index", report.commit_index, colorize_if(report.commit_index != report.last_log_index, CLIColor::Magenta));
	
	render_top_int("Last log term", report.last_log_term, CLIColor::Default);
	
	render_top_int("Current term", report.current_term, CLIColor::Default);
	
	render_top_int("Voted for", report.voted_for.get(), CLIColor::Default);
	
	render_top_str("Since activity", duration_to_string(time_since_last_activity).c_str(), CLIColor::Default);
	
	render_top_str("Random timeout", duration_to_string(report.randomized_timeout).c_str(), CLIColor::Default);
	
	render_top_int("Unknown nodes", report.num_unknown_nodes, colorize_if(report.num_unknown_nodes != 0, CLIColor::Magenta));
	
	render_top_int("Node failures", node->getInternalFailureCount(), colorize_if(node->getInternalFailureCount() != 0, CLIColor::Magenta));
	
	const bool all_allocated = server.guessIfAllDynamicNodesAreAllocated();
	render_top_str("All allocated", all_allocated ? "Yes" : "No", colorize_if(!all_allocated, CLIColor::Magenta));
	...
	{	
		if (report.state != RaftCore::ServerStateLeader)
		{	return CLIColor::Default;}
		if (!report.followers[i].node_id.isValid())
		{	return CLIColor::Red;}
	...
	render_followers_state("Next index", [&](std::uint8_t i)
	{	return report.followers[i].next_index;}, follower_color_getter);
	
	render_followers_state("Match index", [&](std::uint8_t i)
	...
	const int num_events_to_render = static_cast<int>(num_rows) - static_cast<int>(next_relevant_event_index) - static_cast<int>(NumRowsWithoutEvents) - 1; // This allows to keep the last line empty for stdout or UAVCAN_TRACE()
	for (int i = 0; i < num_events_to_render && i < static_cast<int>(event_tracer.getNumEvents()); i++)
	{
...
void runForever(const uavcan_linux::NodePtr& node, const std::uint8_t cluster_size, const std::string& event_log_file, const std::string& persistent_storage_path)
{
	/*
	 * Event tracer
	 */
...
Options parseOptions(int argc, const char** argv)
{
	...
	while (argc-- > 0)
	{
		...
		else if (token[1] == 'c')
		{
			...
			enforce(cluster_size >= 1 && cluster_size <= uavcan::dynamic_node_id_server::distributed::ClusterManager::MaxClusterSize, "Invalid cluster size");
