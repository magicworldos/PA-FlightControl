...
{
	return id_.isValidForDataTypeKind(kind_) && (full_name_ != UAVCAN_NULLPTR) && (*full_name_ != '\0');
}
...
std::string DataTypeDescriptor::toString() const
{
	...
	char buf[128];
	(void) snprintf(buf, sizeof(buf), "%s:%u%c:%016llx", full_name_, static_cast<unsigned>(id_.get()), kindch, static_cast<unsigned long long>(signature_.get()));
...
{
	return (kind_ == rhs.kind_) && (id_ == rhs.id_) && (signature_ == rhs.signature_) && !std::strncmp(full_name_, rhs.full_name_, MaxFullNameLen);
}
