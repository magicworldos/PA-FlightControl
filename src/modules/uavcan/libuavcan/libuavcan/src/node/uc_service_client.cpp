...
{
	UAVCAN_TRACE("ServiceClient::CallState", "Timeout from nid=%d, tid=%d, dtname=%s", int(id_.server_node_id.get()), int(id_.transfer_id.get()), (owner_.data_type_descriptor_ == UAVCAN_NULLPTR) ? "???" : owner_.data_type_descriptor_->getFullName());
	/*
...
int ServiceClientBase::prepareToCall(INode& node, const char* dtname, NodeID server_node_id, ServiceCallID& out_call_id)
{
	 ...
	 */
	const OutgoingTransferRegistryKey otr_key(data_type_descriptor_->getID(), TransferTypeServiceRequest, server_node_id);
	const MonotonicTime otr_deadline = node.getMonotonicTime() + TransferSender::getDefaultMaxTransferInterval();
	TransferID* const otr_tid = node.getDispatcher().getOutgoingTransferRegistry().accessOrCreate(otr_key, otr_deadline);
	if (!otr_tid)
	{
