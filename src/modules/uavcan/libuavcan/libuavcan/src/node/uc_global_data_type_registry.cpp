...
GlobalDataTypeRegistry::RegistrationResult GlobalDataTypeRegistry::registImpl(Entry* dtd)
{
	...
	{   // Collision check
		...
		while (p)
		{
			...
			if (!std::strncmp(p->descriptor.getFullName(), dtd->descriptor.getFullName(), DataTypeDescriptor::MaxFullNameLen))                        // Name collision
			{
				return RegistrationResultCollision;
			}
			p = p->getNextListNode();
...
void GlobalDataTypeRegistry::freeze()
{
	...
	{
		frozen_ = true;
		UAVCAN_TRACE("GlobalDataTypeRegistry", "Frozen; num msgs: %u, num srvs: %u", getNumMessageTypes(), getNumServiceTypes());
	}
}
