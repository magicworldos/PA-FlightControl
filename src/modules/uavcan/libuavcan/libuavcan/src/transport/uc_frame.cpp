...
bool Frame::compile(CanFrame& out_can_frame) const
{
	 ...
	 * CAN ID field
	 */
	out_can_frame.id = CanFrame::FlagEFF | bitpack<0, 7>(src_node_id_.get()) | bitpack<24, 5>(transfer_priority_.get());
	
	if (transfer_type_ == TransferTypeMessageBroadcast)
	{
		out_can_frame.id |= bitpack<7, 1>(0U) | bitpack<8, 16>(data_type_id_.get());
	}
	else
	{
		const bool request_not_response = transfer_type_ == TransferTypeServiceRequest;
		out_can_frame.id |= bitpack<7, 1>(1U) | bitpack<8, 7>(dst_node_id_.get()) | bitpack<15, 1>(request_not_response ? 1U : 0U) | bitpack<16, 8>(data_type_id_.get());
	}
...
bool Frame::isValid() const
{
	...
	// Anonymous transfers
	if (src_node_id_.isBroadcast() && (!start_of_transfer_ || !end_of_transfer_ || (transfer_type_ != TransferTypeMessageBroadcast)))
	{
		UAVCAN_TRACE("Frame", "Validness check failed at line %d", __LINE__);
		return false;
	}
...
std::string Frame::toString() const
{
	...
	char buf[BUFLEN];
	int ofs = snprintf(buf, BUFLEN, "prio=%d dtid=%d tt=%d snid=%d dnid=%d sot=%d eot=%d togl=%d tid=%d payload=[", int(transfer_priority_.get()), int(data_type_id_.get()), int(transfer_type_), int(src_node_id_.get()), int(dst_node_id_.get()), int(start_of_transfer_), int(end_of_transfer_), int(toggle_), int(transfer_id_.get()));
