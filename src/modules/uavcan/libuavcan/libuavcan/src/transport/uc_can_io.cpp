...
int CanIOManager::sendToIface(uint8_t iface_index, const CanFrame& frame, MonotonicTime tx_deadline, CanIOFlags flags)
{
	...
	if (res != 1)
	{
		UAVCAN_TRACE("CanIOManager", "Send failed: code %i, iface %i, frame %s", res, iface_index, frame.toString().c_str());
	}
	if (res > 0)
	{
...
int CanIOManager::callSelect(CanSelectMasks& inout_masks, const CanFrame* (&pending_tx)[MaxCanIfaces], MonotonicTime blocking_deadline)
{
		    ...
		    num_ifaces_(driver.getNumIfaces())
{
	...
	UAVCAN_TRACE("CanIOManager", "Memory blocks per iface: %u, total: %u", unsigned(mem_blocks_per_iface), unsigned(allocator.getBlockCapacity()));
	
	for (int i = 0; i < num_ifaces_; i++)
	{
		tx_queues_[i].construct<IPoolAllocator&, ISystemClock&, std::size_t>(allocator, sysclock, mem_blocks_per_iface);
	}
}
...
int CanIOManager::send(const CanFrame& frame, MonotonicTime tx_deadline, MonotonicTime blocking_deadline, uint8_t iface_mask, CanTxQueue::Qos qos, CanIOFlags flags)
{
