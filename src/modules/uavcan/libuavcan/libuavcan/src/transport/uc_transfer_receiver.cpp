...
bool TransferReceiver::validate(const RxFrame& frame) const
{
	...
	{
		UAVCAN_TRACE("TransferReceiver", "Unexpected toggle bit (not %i), %s", int(next_toggle_), frame.toString().c_str());
		registerError();
		return false;
	}
...
{
	if ((frame.getMonotonicTimestamp().isZero()) || (frame.getMonotonicTimestamp() < prev_transfer_ts_) || (frame.getMonotonicTimestamp() < this_transfer_ts_))
	{
	...
	// FSM, the hard way
	const bool need_restart = (not_initialized) || (tid_timed_out) || (same_iface && first_frame && not_previous_tid) || (iface_switch_allowed && first_frame && non_wrapped_tid);
	...
	if (need_restart)
	{
		...
		UAVCAN_TRACE("TransferReceiver", "Restart [ni=%d, isa=%d, tt=%d, si=%d, ff=%d, nwtid=%d, nptid=%d, tid=%d], %s", int(not_initialized), int(iface_switch_allowed), int(tid_timed_out), int(same_iface), int(first_frame), int(non_wrapped_tid), int(not_previous_tid), int(tid_.get()), frame.toString().c_str());
		tba.remove();
