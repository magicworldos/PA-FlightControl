...
int16_t CanAcceptanceFilterConfigurator::mergeConfigurations()
{
	...
	while (acceptance_filters_number < multiset_configs_.getSize())
	{
		...
		for (uint16_t i_ind = 0; i_ind < multiset_array_size - 1; i_ind++)
		{
			...
			{
				CanFilterConfig temp_config = mergeFilters(*multiset_configs_.getByIndex(i_ind), *multiset_configs_.getByIndex(j_ind));
		...
		*multiset_configs_.getByIndex(j_rank) = mergeFilters(*multiset_configs_.getByIndex(i_rank), *multiset_configs_.getByIndex(j_rank));
...
int CanAcceptanceFilterConfigurator::applyConfiguration(void)
{
	...
	for (uint8_t i = 0; i < node_.getDispatcher().getCanIOManager().getNumIfaces(); i++)
	{
		...
		int16_t num = iface->configureFilters(reinterpret_cast<CanFilterConfig*>(&filter_conf_array), static_cast<uint16_t>(filter_array_size));
		if (num < 0)
		{
			UAVCAN_TRACE("CanAcceptanceFilter", "Failed to apply HW filter configuration");
			return -ErrDriver;
