...
SingleFrameIncomingTransfer::SingleFrameIncomingTransfer(const RxFrame& frm) :
		    IncomingTransfer(frm.getMonotonicTimestamp(), frm.getUtcTimestamp(), frm.getPriority(), frm.getTransferType(), frm.getTransferID(), frm.getSrcNodeID(), frm.getIfaceIndex()),
 ...
 */
MultiFrameIncomingTransfer::MultiFrameIncomingTransfer(MonotonicTime ts_mono, UtcTime ts_utc, const RxFrame& last_frame, TransferBufferAccessor& tba) :
		    IncomingTransfer(ts_mono, ts_utc, last_frame.getPriority(), last_frame.getTransferType(), last_frame.getTransferID(), last_frame.getSrcNodeID(), last_frame.getIfaceIndex()),
		    buf_acc_(tba)
{
 ...
 */
bool TransferListener::TimedOutReceiverPredicate::operator()(const TransferBufferManagerKey& key, const TransferReceiver& value) const
{
...
bool TransferListener::checkPayloadCrc(const uint16_t compare_with, const ITransferBuffer& tbb) const
{
	...
	{
		UAVCAN_TRACE("TransferListener", "CRC mismatch, expected=0x%04x, got=0x%04x", int(compare_with), int(crc.get()));
		return false;
	}
	return true;
}

void TransferListener::handleReception(TransferReceiver& receiver, const RxFrame& frame, TransferBufferAccessor& tba)
{
	switch (receiver.addFrame(frame, tba))
	{
		...
		case TransferReceiver::ResultComplete:
		{
			...
			MultiFrameIncomingTransfer it(receiver.getLastTransferTimestampMonotonic(), receiver.getLastTransferTimestampUtc(), frame, tba);
			handleIncomingTransfer(it);
			it.release();
			break;
		}
...
void TransferListener::handleFrame(const RxFrame& frame)
{
	...
	else if (frame.getSrcNodeID().isBroadcast() && frame.isStartOfTransfer() && frame.isEndOfTransfer() && frame.getDstNodeID().isBroadcast())        // Anonymous transfer
	{
