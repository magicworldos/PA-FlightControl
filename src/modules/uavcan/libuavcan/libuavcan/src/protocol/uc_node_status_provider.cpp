...
void NodeStatusProvider::handleGetNodeInfoRequest(const protocol::GetNodeInfo::Request&, protocol::GetNodeInfo::Response& rsp)
{
...
void NodeStatusProvider::setStatusPublicationPeriod(uavcan::MonotonicDuration period)
{
	...
	UAVCAN_TRACE("NodeStatusProvider", "Status pub period: %s, TX timeout: %s", period.toString().c_str(), node_status_pub_.getTxTimeout().toString().c_str());
}
