...
MonotonicDuration DynamicNodeIDClient::getRandomDuration(uint32_t lower_bound_msec, uint32_t upper_bound_msec)
{
	...
	// coverity[dont_call]
	return MonotonicDuration::fromMSec(lower_bound_msec + static_cast<uint32_t>(std::rand()) % (upper_bound_msec - lower_bound_msec));
}
...
void DynamicNodeIDClient::restartTimer(const Mode mode)
{
	...
	UAVCAN_TRACE("DynamicNodeIDClient", "Restart mode %d, delay %d ms", static_cast<int>(mode), static_cast<int>(delay.toMSec()));
}
...
void DynamicNodeIDClient::handleTimerEvent(const TimerEvent&)
{
	 ...
	 */
	UAVCAN_TRACE("DynamicNodeIDClient", "Broadcasting; preferred ID %d, size of UID %d", static_cast<int>(preferred_node_id_.get()), static_cast<int>(tx.unique_id.size()));
...
void DynamicNodeIDClient::handleAllocation(const ReceivedDataStructure<protocol::dynamic_node_id::Allocation>& msg)
{
	...
	UAVCAN_TRACE("DynamicNodeIDClient", "Allocation message from %d, %d bytes of unique ID, node ID %d", static_cast<int>(msg.getSrcNodeID().get()), static_cast<int>(msg.unique_id.size()), static_cast<int>(msg.node_id));
	...
	if (msg.isAnonymousTransfer() || msg.unique_id.empty() || (full_response && (msg.node_id == 0)))
	{
	...
	if (equal(msg.unique_id.begin(), msg.unique_id.end(), unique_id_))
	{
		if (full_response)
		{
			...
			UAVCAN_TRACE("DynamicNodeIDClient", "Allocation complete, node ID %d provided by %d", static_cast<int>(allocated_node_id_.get()), static_cast<int>(allocator_node_id_.get()));
		}
		else
		{
...
int DynamicNodeIDClient::start(const UniqueID& unique_id, const NodeID preferred_node_id, const TransferPriority transfer_priority)
{
	terminate();
