...
public:
	...
	{
		return getScheduler().getDispatcher();
	}
	...
	{
		return getScheduler().getSystemClock();
	}
	MonotonicTime getMonotonicTime() const
	{
		return getScheduler().getMonotonicTime();
	}
	UtcTime getUtcTime() const
	{
		return getScheduler().getUtcTime();
	}
	...
	{
		return getScheduler().getDispatcher().getNodeID();
	}
	...
	{
		getDispatcher().removeRxFrameListener();
	}
	void installRxFrameListener(IRxFrameListener* lst)
	{
		getDispatcher().installRxFrameListener(lst);
	}
#endif
};
