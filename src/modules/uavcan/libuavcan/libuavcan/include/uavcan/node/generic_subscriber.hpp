...
protected:
	...
	{
		UAVCAN_TRACE("GenericSubscriber", "Start as service request listener; dtname=%s", DataSpec::getDataTypeFullName());
		return genericStart(&Dispatcher::registerServiceRequestListener);
	}
	
	int startAsServiceResponseListener()
	{
		UAVCAN_TRACE("GenericSubscriber", "Start as service response listener; dtname=%s", DataSpec::getDataTypeFullName());
...
int GenericSubscriber<DataSpec, DataStruct, TransferListenerType>::checkInit()
{
	...
	const DataTypeDescriptor* const descr = GlobalDataTypeRegistry::instance().find(DataTypeKind(DataSpec::DataTypeKind), DataSpec::getDataTypeFullName());
	...
	forwarder_.template construct<SelfType&, const DataTypeDescriptor&, uint16_t, IPoolAllocator&>(*this, *descr, MaxBufferSize, node_.getAllocator());
...
void GenericSubscriber<DataSpec, DataStruct, TransferListenerType>::handleIncomingTransfer(IncomingTransfer& transfer)
{
	...
	if (decode_res <= 0)
	{
		UAVCAN_TRACE("GenericSubscriber", "Unable to decode the message [%i] [%s]", decode_res, DataSpec::getDataTypeFullName());
		failure_count_++;
...
template<typename DataSpec, typename DataStruct, typename TransferListenerType>
int GenericSubscriber<DataSpec, DataStruct, TransferListenerType>::genericStart(bool (Dispatcher::*registration_method)(TransferListener*))
{
