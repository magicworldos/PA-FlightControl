...
public:
	...
	{
		return MonotonicDuration::fromUSec(200);
	}
...
class UAVCAN_EXPORT GenericPublisher: public GenericPublisherBase
{
	...
	typedef typename Select<DataStruct::MaxBitLen == 0, ZeroTransferBuffer, StaticTransferBuffer<BitLenToByteLen<DataStruct::MaxBitLen>::Result> >::Result Buffer;

	enum
	{
		Qos = (DataTypeKind(DataSpec::DataTypeKind) == DataTypeKindMessage) ? CanTxQueue::Volatile : CanTxQueue::Persistent
	};
...
template<typename DataSpec, typename DataStruct>
int GenericPublisher<DataSpec, DataStruct>::genericPublish(const DataStruct& message, TransferType transfer_type, NodeID dst_node_id, TransferID* tid, MonotonicTime blocking_deadline)
{
