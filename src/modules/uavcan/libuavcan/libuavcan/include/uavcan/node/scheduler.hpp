...
public:
	...
	{
		startWithDeadline(MonotonicTime::fromUSec(1));
	}
...
class UAVCAN_EXPORT Scheduler: Noncopyable
{
	enum
	{
		DefaultDeadlineResolutionMs = 5
	};
	enum
	{
		MinDeadlineResolutionMs = 1
	};
	enum
	{
		MaxDeadlineResolutionMs = 100
	};

	enum
	{
		DefaultCleanupPeriodMs = 1000
	};
	enum
	{
		MinCleanupPeriodMs = 10
	};
	enum
	{
		MaxCleanupPeriodMs = 10000
	};
...
public:
	...
	{
		return dispatcher_.getSystemClock();
	}
	MonotonicTime getMonotonicTime() const
	{
		return dispatcher_.getSystemClock().getMonotonic();
	}
	UtcTime getUtcTime() const
	{
		return dispatcher_.getSystemClock().getUtc();
	}
