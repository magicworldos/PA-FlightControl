...
struct ServiceCallID
{
	...
	{
		return (rhs.server_node_id == server_node_id) && (rhs.transfer_id == transfer_id);
	}
	
	bool isValid() const
	{
		return server_node_id.isUnicast();
	}
};
 ...
 * Do not use directly.
 */
class ServiceClientBase: protected ITransferAcceptanceFilter, protected DeadlineHandler
{
...
protected:
	...
	struct CallStateMatchingPredicate
	{
		...
		{
			return (state.getCallID() == id) && !state.hasTimedOut();
		}
	};
	...
	struct ServerSearchPredicate
	{
		...
		bool operator()(const CallState& state) const
		{
			return state.getCallID().server_node_id == server_node_id;
		}
	};
...
public:
	...
	{
		return MonotonicDuration::fromMSec(10);
	}
	static MonotonicDuration getMaxRequestTimeout()
	{
		return MonotonicDuration::fromMSec(60000);
	}
};
...
private:
	...
	struct TimeoutCallbackCaller
	{
		...
		void operator()(const CallState& state)
		{
			...
			{
				UAVCAN_TRACE("ServiceClient::TimeoutCallbackCaller", "Timeout from nid=%d, tid=%d, dtname=%s", int(state.getCallID().server_node_id.get()), int(state.getCallID().transfer_id.get()), DataType::getDataTypeFullName());
				
				typename SubscriberType::ReceivedDataStructureSpec rx_struct; // Default-initialized
				
				ServiceCallResultType result(ServiceCallResultType::ErrorTimeout, state.getCallID(), rx_struct);    // Mutable!
...
public:
	...
	{
		return publisher_.getPriority();
	}
	void setPriority(const TransferPriority prio)
	{
		publisher_.setPriority(prio);
	}
};
...
int ServiceClient<DataType_, Callback_>::addCallState(ServiceCallID call_id)
{
	...
	if (UAVCAN_NULLPTR == call_registry_.template emplace<INode&, ServiceClientBase&, ServiceCallID>(SubscriberType::getNode(), *this, call_id))
	{
		SubscriberType::stop();
		return -ErrMemory;
	}
...
template<typename DataType_, typename Callback_>
int ServiceClient<DataType_, Callback_>::call(NodeID server_node_id, const RequestType& request, ServiceCallID& out_call_id)
{
	if (!coerceOrFallback<bool>(callback_, true))
	{
