 ...
 */
class UAVCAN_EXPORT Server: public AbstractServer, IRaftLeaderMonitor
{
	...
	virtual void handleAllocationRequest(const UniqueID& unique_id, const NodeID preferred_node_id)
	{
		 ...
		 */
		const LazyConstructor<RaftCore::LogEntryInfo> result = raft_core_.traverseLogFromEndUntil(UniqueIDLogPredicate(unique_id));
		...
		if (result.isConstructed())
		{
			if (result->committed)
			{
				...
				UAVCAN_TRACE("dynamic_node_id_server::distributed::Server", "Allocation request served with existing allocation; node ID %d", int(result->entry.node_id));
			}
			else
			{
				UAVCAN_TRACE("dynamic_node_id_server::distributed::Server", "Allocation request ignored - allocation exists but not committed yet; node ID %d", int(result->entry.node_id));
			}
		}
		else
		{
	...
	{
		const LazyConstructor<RaftCore::LogEntryInfo> result = raft_core_.traverseLogFromEndUntil(NodeIDLogPredicate(node_id));
		if (result.isConstructed())
		{
	...
	virtual void handleLocalLeadershipChange(bool local_node_is_leader)
	{
		...
		const LazyConstructor<RaftCore::LogEntryInfo> result = raft_core_.traverseLogFromEndUntil(NodeIDLogPredicate(node_.getNodeID()));
	...
	{
		UAVCAN_TRACE("dynamic_node_id_server::distributed::Server", "Testing if node ID %d is taken", int(node_id.get()));
	...
	{
		const NodeID allocated_node_id = NodeIDSelector<Server>(this, &Server::isNodeIDTaken).findFreeNodeID(preferred_node_id);
		...
		UAVCAN_TRACE("dynamic_node_id_server::distributed::Server", "New node ID allocated: %d", int(allocated_node_id.get()));
...
public:
	...
	int init(const UniqueID& own_unique_id, const uint8_t cluster_size = ClusterManager::ClusterSizeUnknown, const TransferPriority priority = TransferPriority::OneHigherThanLowest)
	{
		 ...
		 */
		const LazyConstructor<RaftCore::LogEntryInfo> own_log_entry = raft_core_.traverseLogFromEndUntil(NodeIDLogPredicate(node_.getNodeID()));
	...
	{
		return raft_core_.getNumAllocations();
	}
