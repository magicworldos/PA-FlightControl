 ...
 */
class FirmwareUpdateTrigger: public INodeInfoListener, private TimerBase
{
	...
	};   ///< Shall not be less than default service response timeout.
	...
	virtual void handleNodeInfoRetrieved(const NodeID node_id, const protocol::GetNodeInfo::Response& node_info)
	{
		...
		if (update_needed)
		{
			UAVCAN_TRACE("FirmwareUpdateTrigger", "Node ID %d requires update; file path: %s", int(node_id.get()), firmware_file_path.c_str());
	...
	INode& getNode()
	{
		return begin_fw_update_client_.getNode();
	}
	...
	NodeID pickNextNodeID() const
	{
		...
		UAVCAN_TRACE("FirmwareUpdateTrigger", "Next node ID to query: %d, pending nodes: %u, pending calls: %u", int(last_queried_node_id_), pending_nodes_.getSize(), begin_fw_update_client_.getNumPendingCalls());
	...
	void handleBeginFirmwareUpdateResponse(const ServiceCallResult<protocol::file::BeginFirmwareUpdate>& result)
	{
		...
		{
			UAVCAN_TRACE("FirmwareUpdateTrigger", "Request to %d has timed out, will retry", int(result.getCallID().server_node_id.get()));
			return;
		}
		...
		if (confirmed)
		{
			UAVCAN_TRACE("FirmwareUpdateTrigger", "Node %d confirmed the update request", int(result.getCallID().server_node_id.get()));
		...
		else
		{
			...
			const bool update_needed = checker_.shouldRetryFirmwareUpdate(result.getCallID().server_node_id, result.getResponse(), *old_path);
			
			if (!update_needed)
			{
				UAVCAN_TRACE("FirmwareUpdateTrigger", "Node %d does not need retry", int(result.getCallID().server_node_id.get()));
				pending_nodes_.remove(result.getCallID().server_node_id);
			}
		}
	}
	...
	virtual void handleTimerEvent(const TimerEvent&)
	{
		...
		UAVCAN_TRACE("FirmwareUpdateTrigger", "Request to %d with path: %s", int(node_id.get()), req.image_file_remote_path.path.c_str());
...
public:
	...
	int start(NodeInfoRetriever& node_info_retriever, const FirmwareFilePath& arg_common_path_prefix = FirmwareFilePath(), const TransferPriority priority = TransferPriority::OneHigherThanLowest)
	{
		...
		if (!common_path_prefix_.empty() && *(common_path_prefix_.end() - 1) == protocol::file::Path::SEPARATOR)
		{
