...
private:
	typedef MethodBinder<RaftCore*, void (RaftCore::*)(const ReceivedDataStructure<AppendEntries::Request>&, ServiceResponseDataStructure<AppendEntries::Response>&)> AppendEntriesCallback;

	typedef MethodBinder<RaftCore*, void (RaftCore::*)(const ServiceCallResult<AppendEntries>&)> AppendEntriesResponseCallback;

	typedef MethodBinder<RaftCore*, void (RaftCore::*)(const ReceivedDataStructure<RequestVote::Request>&, ServiceResponseDataStructure<RequestVote::Response>&)> RequestVoteCallback;

	typedef MethodBinder<RaftCore*, void (RaftCore::*)(const ServiceCallResult<RequestVote>&)> RequestVoteResponseCallback;
	...
	enum
	{
		MaxNumFollowers = ClusterManager::MaxClusterSize - 1
	};
	...
	{
		tracer_.onEvent(event, argument);
	}
	
	INode& getNode()
	{
		return append_entries_srv_.getNode();
	}
	...
	void checkInvariants() const
	{
		...
		// Term
		UAVCAN_ASSERT(persistent_state_.getLog().getEntryAtIndex(persistent_state_.getLog().getLastIndex()) != UAVCAN_NULLPTR);
		UAVCAN_ASSERT(persistent_state_.getLog().getEntryAtIndex(persistent_state_.getLog().getLastIndex())->term <= persistent_state_.getCurrentTerm());
		
		// Elections
		UAVCAN_ASSERT(server_state_ != ServerStateCandidate || !request_vote_client_.hasPendingCalls() || persistent_state_.getVotedFor() == getNode().getNodeID());
		...
		UAVCAN_ASSERT(server_state_ != ServerStateFollower || (!append_entries_client_.hasPendingCalls() && !request_vote_client_.hasPendingCalls()));
	}
	...
	void registerActivity()
	{
		...
		static const int32_t randomization_range_msec = AppendEntries::Request::DEFAULT_MAX_ELECTION_TIMEOUT_MS - AppendEntries::Request::DEFAULT_MIN_ELECTION_TIMEOUT_MS;
		// coverity[dont_call]
		const int32_t random_msec = (std::rand() % randomization_range_msec) + 1;
		
		randomized_activity_timeout_ = MonotonicDuration::fromMSec(AppendEntries::Request::DEFAULT_MIN_ELECTION_TIMEOUT_MS + random_msec);
	...
	void updateCandidate()
	{
		...
		else
		{
			...
			for (uint8_t i = 0; i < MaxNumFollowers; i++)
			{
				...
				UAVCAN_TRACE("dynamic_node_id_server::distributed::RaftCore", "Requesting vote from %d", int(node_id.get()));
	...
	void updateLeader()
	{
		...
		if (cluster_.getClusterSize() > 1)
		{
			...
			for (Log::Index index = cluster_.getServerNextIndex(node_id); index <= persistent_state_.getLog().getLastIndex(); index++)
			{
	...
	void switchState(ServerState new_state)
	{
		 ...
		 * Logging
		 */
		UAVCAN_TRACE("dynamic_node_id_server::distributed::RaftCore", "State switch: %d --> %d", int(server_state_), int(new_state));
		 ...
		 */
		if ((old_state == ServerStateLeader) || (new_state == ServerStateLeader))
		{
	...
	void handleRequestVoteRequest(const ReceivedDataStructure<RequestVote::Request>& request, ServiceResponseDataStructure<RequestVote::Response>& response)
	{
		...
		{
			const bool can_vote = !persistent_state_.isVotedForSet() || (persistent_state_.getVotedFor() == request.getSrcNodeID());
			const bool log_is_up_to_date = persistent_state_.getLog().isOtherLogUpToDate(request.last_log_index, request.last_log_term);
...
public:
	...
	int init(const uint8_t cluster_size, const TransferPriority priority)
	{
		...
		append_entries_client_.setRequestTimeout(min(append_entries_client_.getDefaultRequestTimeout(), update_interval));
		
		request_vote_client_.setRequestTimeout(min(request_vote_client_.getDefaultRequestTimeout(), update_interval));
	...
	{
		return commit_index_ == persistent_state_.getLog().getLastIndex();
	}
	...
	{
		return randomized_activity_timeout_;
	}
};
