...
class UAVCAN_EXPORT GlobalTimeSyncMaster: protected LoopbackFrameListenerBase
{
	...
	public:
		...
		int publish(TransferID tid, MonotonicTime current_time)
		{
			...
			UAVCAN_TRACE("GlobalTimeSyncMaster", "Publishing %llu iface=%i tid=%i", static_cast<unsigned long long>(msg.previous_transmission_timestamp_usec), int(iface_index_), int(tid.get()));
	...
	{
		const MonotonicDuration max_transfer_interval = MonotonicDuration::fromMSec(protocol::GlobalTimeSync::MAX_BROADCASTING_PERIOD_MS);
		...
		TransferID* const tid_ptr = node_.getDispatcher().getOutgoingTransferRegistry().accessOrCreate(otr_key, otr_deadline);
...
public:
	...
	int init(const TransferPriority priority = TransferPriority::OneLowerThanHighest)
	{
		...
		// Data type ID
		const DataTypeDescriptor* const desc = GlobalDataTypeRegistry::instance().find(DataTypeKindMessage, protocol::GlobalTimeSync::getDataTypeFullName());
