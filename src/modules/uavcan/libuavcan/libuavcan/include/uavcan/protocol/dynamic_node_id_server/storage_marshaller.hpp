...
public:
	...
	int get(const IStorageBackend::String& key, UniqueID& out_value) const
	{
		...
		for (IStorageBackend::String::const_iterator it = val.begin(); it != val.end(); ++it)
		{
			if ((static_cast<char>(*it) < '0' || static_cast<char>(*it) > '9') && (static_cast<char>(*it) < 'a' || static_cast<char>(*it) > 'f'))
			{
				return -ErrFailure;
			}
		}
		...
		for (uint8_t byte_index = 0; byte_index < NumBytes; byte_index++)
		{
			...
			out_value[byte_index] = static_cast<uint8_t>(convertLowerCaseHexCharToNibble(static_cast<char>(*it++)) | out_value[byte_index]);
		}
