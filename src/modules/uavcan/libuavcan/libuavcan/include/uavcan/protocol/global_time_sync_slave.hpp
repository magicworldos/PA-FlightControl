...
class UAVCAN_EXPORT GlobalTimeSyncSlave: Noncopyable
{
	...
	{
		return sub_.getNode().getSystemClock();
	}
	...
	void adjustFromMsg(const ReceivedDataStructure<protocol::GlobalTimeSync>& msg)
	{
		...
		UAVCAN_TRACE("GlobalTimeSyncSlave", "Adjustment: usec=%lli snid=%i iface=%i suppress=%i", static_cast<long long>(adjustment.toUSec()), int(msg.getSrcNodeID().get()), int(msg.getIfaceIndex()), int(suppressed_));
	...
	{
		UAVCAN_TRACE("GlobalTimeSyncSlave", "Update: snid=%i iface=%i", int(msg.getSrcNodeID().get()), int(msg.getIfaceIndex()));
	...
	void processMsg(const ReceivedDataStructure<protocol::GlobalTimeSync>& msg)
	{
		...
		{
			UAVCAN_TRACE("GlobalTimeSyncSlave", "Force update: needs_init=%i switch_master=%i pub_timeout=%i", int(needs_init), int(switch_master), int(pub_timeout));
			updateFromMsg(msg);
		}
		else if (msg.getIfaceIndex() == prev_iface_index_ && msg.getSrcNodeID() == master_nid_)
		{
			if (state_ == Adjust)
			{
				...
				{
					UAVCAN_TRACE("GlobalTimeSyncSlave", "Adjustment skipped: msg_invalid=%i wrong_tid=%i wrong_timing=%i", int(msg_invalid), int(wrong_tid), int(wrong_timing));
					state_ = Update;
				}
			}
			if (state_ == Adjust)
		...
		{
			UAVCAN_TRACE("GlobalTimeSyncSlave", "Ignored: snid=%i iface=%i", int(msg.getSrcNodeID().get()), int(msg.getIfaceIndex()));
		}
	}
...
public:
	...
	bool isActive() const
	{
		...
		return !last_adjustment_ts_.isZero() && (since_prev_adj.toMSec() <= protocol::GlobalTimeSync::RECOMMENDED_BROADCASTER_TIMEOUT_MS);
	}
