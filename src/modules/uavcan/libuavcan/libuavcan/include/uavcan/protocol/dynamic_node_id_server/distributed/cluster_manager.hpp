...
public:
	enum
	{
		MaxClusterSize = Discovery::FieldTypes::known_nodes::MaxSize
	};

private:
	typedef MethodBinder<ClusterManager*, void (ClusterManager::*)(const ReceivedDataStructure<Discovery>&)> DiscoveryCallback;
	...
	virtual void handleTimerEvent(const TimerEvent&)
	{
		 ...
		 * Broadcasting
		 */
		UAVCAN_TRACE("dynamic_node_id_server::distributed::ClusterManager", "Broadcasting Discovery message; known nodes: %d of %d", int(msg.known_nodes.size()), int(cluster_size_));
		...
		{
			UAVCAN_TRACE("dynamic_node_id_server::distributed::ClusterManager", "Discovery broadcasting timer stopped");
			stop();
		}
	}
...
public:
	...
	int init(const uint8_t init_cluster_size, const TransferPriority priority)
	{
		...
		if (init_cluster_size == ClusterSizeUnknown)
		{
			...
			if (res < 0)
			{
				UAVCAN_TRACE("dynamic_node_id_server::distributed::ClusterManager", "Cluster size is neither configured nor stored in the storage");
				return res;
			}
	...
	{
		return static_cast<uint8_t>(cluster_size_ / 2U + 1U);
	}
	
	bool isClusterDiscovered() const
	{
		return num_known_servers_ == (cluster_size_ - 1);
	}
};
