...
public:
	...
	int init()
	{
		...
		for (Index index = 0; index <= last_index_; index++)
		{
			...
			if (result < 0)
			{
				UAVCAN_TRACE("dynamic_node_id_server::distributed::Log", "Failed to read entry at index %u: %d", unsigned(index), result);
				return result;
			}
		}
	...
	int append(const Entry& entry)
	{
		...
		UAVCAN_TRACE("dynamic_node_id_server::distributed::Log", "New entry, index %u, node ID %u, term %u", unsigned(last_index_), unsigned(entry.node_id), unsigned(entry.term));
		return 0;
	}
	...
	int removeEntriesWhereIndexGreaterOrEqual(Index index)
	{
		...
		if (new_last_index != last_index_)
		{
			...
			UAVCAN_TRACE("dynamic_node_id_server::distributed::Log", "Entries removed, last index %u --> %u", unsigned(last_index_), unsigned(new_last_index));
			last_index_ = Index(new_last_index);
		}
