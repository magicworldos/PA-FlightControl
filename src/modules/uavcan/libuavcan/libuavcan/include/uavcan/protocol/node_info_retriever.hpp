 ...
 */
class UAVCAN_EXPORT NodeInfoRetriever: public NodeStatusMonitor, TimerBase
{
public:
	enum
	{
		MaxNumRequestAttempts = 254
	};
	enum
	{
		UnlimitedRequestAttempts = 0
	};
...
private:
	...
	enum
	{
		DefaultNumRequestAttempts = 16
	};
	enum
	{
		DefaultTimerIntervalMSec = 40
	};  ///< Read explanation in the class documentation
	...
	{
		return const_cast<NodeInfoRetriever*>(this)->getEntry(node_id);
	}
	...
	NodeID pickNextNodeToQuery(bool& out_at_least_one_request_needed) const
	{
		...
		for (unsigned iter_cnt_ = 0; iter_cnt_ < (sizeof(entries_) / sizeof(entries_[0])); iter_cnt_++) // Round-robin
		{
			...
			UAVCAN_ASSERT((last_picked_node_ >= 1) && (last_picked_node_ <= NodeID::Max));
			...
			if (entry.request_needed)
			{
				...
				if (entry.updated_since_last_attempt && !get_node_info_client_.hasPendingCallToServer(last_picked_node_))
				{
	...
	{
		const bool was_offline = !event.was_known || (event.old_status.mode == protocol::NodeStatus::MODE_OFFLINE);
		...
		if (was_offline || offline_now)
		{
			...
			UAVCAN_TRACE("NodeInfoRetriever", "Offline status change: node ID %d, request needed: %d", int(event.node_id.get()), int(entry.request_needed));
		...
		listeners_.forEach(GenericHandlerCaller<const NodeStatusChangeEvent&>(&INodeInfoListener::handleNodeStatusChange, event));
	}
	...
	virtual void handleNodeStatusMessage(const ReceivedDataStructure<protocol::NodeStatus>& msg)
	{
		...
		listeners_.forEach(GenericHandlerCaller<const ReceivedDataStructure<protocol::NodeStatus>&>(&INodeInfoListener::handleNodeStatusMessage, msg));
	}
	...
	void handleGetNodeInfoResponse(const ServiceCallResult<protocol::GetNodeInfo>& result)
	{
		...
		if (result.isSuccessful())
		{
			...
			listeners_.forEach(NodeInfoRetrievedHandlerCaller(result.getCallID().server_node_id, result.getResponse()));
		}
		else
		{
			if (num_attempts_ != UnlimitedRequestAttempts)
			{
				...
				{
					entry.request_needed = false;
					listeners_.forEach(GenericHandlerCaller<NodeID>(&INodeInfoListener::handleNodeInfoUnavailable, result.getCallID().server_node_id));
				}
			}
		}
	}
...
public:
	...
	int start(const TransferPriority priority = TransferPriority::OneHigherThanLowest)
	{
		...
		get_node_info_client_.setCallback(GetNodeInfoResponseCallback(this, &NodeInfoRetriever::handleGetNodeInfoResponse));
