...
public:
	...
	int init()
	{
		...
		if (storage_.get(getCurrentTermKey()).empty() && log_is_empty)
		{
			...
			if (res < 0)
			{
				UAVCAN_TRACE("dynamic_node_id_server::distributed::PersistentState", "Failed to init current term: %d", res);
				return res;
			}
			if (current_term_ != 0)
			{
		...
		else
		{
			...
			if (res < 0)
			{
				UAVCAN_TRACE("dynamic_node_id_server::distributed::PersistentState", "Failed to read current term: %d", res);
				return res;
			}
		}
		...
		{
			UAVCAN_TRACE("dynamic_node_id_server::distributed::PersistentState", "Persistent storage is damaged: current term is less than term of the last log entry (%u < %u)", unsigned(current_term_), unsigned(last_entry->term));
			return -ErrLogic;
		}
		...
		if (storage_.get(getVotedForKey()).empty() && log_is_empty && (current_term_ == 0))
		{
			...
			if (res < 0)
			{
				UAVCAN_TRACE("dynamic_node_id_server::distributed::PersistentState", "Failed to init votedFor: %d", res);
				return res;
			}
		...
		else
		{
			...
			if (res < 0)
			{
				UAVCAN_TRACE("dynamic_node_id_server::distributed::PersistentState", "Failed to read votedFor: %d", res);
				return res;
			}
