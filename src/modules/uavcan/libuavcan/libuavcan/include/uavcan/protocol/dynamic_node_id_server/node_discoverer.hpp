...
{
	typedef MethodBinder<NodeDiscoverer*, void (NodeDiscoverer::*)(const ServiceCallResult<protocol::GetNodeInfo>&)> GetNodeInfoResponseCallback;

	typedef MethodBinder<NodeDiscoverer*, void (NodeDiscoverer::*)(const ReceivedDataStructure<protocol::NodeStatus>&)> NodeStatusCallback;
	...
	enum
	{
		MaxAttemptsToGetNodeInfo = 5
	};

	enum
	{
		TimerPollIntervalMs = 170
	}; // ~ ceil(500 ms service timeout / 3)
	...
	{
		tracer_.onEvent(code, argument);
	}
	
	INode& getNode()
	{
		return node_status_sub_.getNode();
	}
	...
	void handleGetNodeInfoResponse(const ServiceCallResult<protocol::GetNodeInfo>& result)
	{
		...
		{
			UAVCAN_TRACE("dynamic_node_id_server::NodeDiscoverer", "GetNodeInfo response from %d", int(result.getCallID().server_node_id.get()));
		...
		else
		{
			...
			UAVCAN_TRACE("dynamic_node_id_server::NodeDiscoverer", "GetNodeInfo request to %d has timed out, %d attempts", int(result.getCallID().server_node_id.get()), int(data->num_get_node_info_attempts));
	...
	void handleTimerEvent(const TimerEvent&)
	{
		...
		UAVCAN_TRACE("dynamic_node_id_server::NodeDiscoverer", "Requesting GetNodeInfo from node %d", int(node_id.get()));
...
public:
	...
	{
		return static_cast<uint8_t>(node_map_.getSize());
	}
};
