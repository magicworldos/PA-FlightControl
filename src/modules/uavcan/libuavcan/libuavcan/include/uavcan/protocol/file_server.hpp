...
public:
	...
	enum
	{
		ReadSize = protocol::file::Read::Response::FieldTypes::data::MaxSize
	};
...
class FileServer: protected BasicFileServer
{
	...
	{
		resp.error.value = backend_.getDirectoryEntryInfo(req.directory_path.path, req.entry_index, resp.entry_type, resp.entry_full_path.path);
	}
...
public:
	...
	int start()
	{
		...
		res = get_directory_entry_info_srv_.start(GetDirectoryEntryInfoCallback(this, &FileServer::handleGetDirectoryEntryInfo));
		if (res < 0)
		{
			return res;
		}
