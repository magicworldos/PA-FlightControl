...
{
	typedef MethodBinder<AllocationRequestManager*, void (AllocationRequestManager::*)(const ReceivedDataStructure<Allocation>&)> AllocationCallback;
	...
	void publishFollowupAllocationResponse()
	{
		...
		UAVCAN_TRACE("AllocationRequestManager", "Intermediate response with %u bytes of unique ID", unsigned(msg.unique_id.size()));
	...
	void handleAllocation(const ReceivedDataStructure<Allocation>& msg)
	{
		...
		{
			UAVCAN_TRACE("AllocationRequestManager", "Allocation request received; preferred node ID: %d", int(msg.node_id));
