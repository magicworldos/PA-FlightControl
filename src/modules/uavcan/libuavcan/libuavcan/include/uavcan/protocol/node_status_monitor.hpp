...
private:
	...
	void changeNodeStatus(const NodeID node_id, const Entry new_entry_value)
	{
		...
		if (entry.status != new_entry_value.status)
		{
			...
			UAVCAN_TRACE("NodeStatusMonitor", "Node %i [%s] status change: [%s] --> [%s]", int(node_id.get()), (event.was_known ? "known" : "new"), event.old_status.toString().c_str(), event.status.toString().c_str());
	...
	void handleTimerEvent(const TimerEvent&)
	{
		...
		for (uint8_t i = 1; i <= NodeID::Max; i++)
		{
			...
			if (entry.time_since_last_update_ms100 >= 0 && entry.status.mode != protocol::NodeStatus::MODE_OFFLINE)
			{
