...
public:
	...
	{
		return fromUSec(NumericTraits<int64_t>::max());
	}
...
void DurationBase<D>::toString(char buf[StringBufSize]) const
{
	...
	(void) snprintf(ptr, StringBufSize - 1, "%llu.%06lu", static_cast<unsigned long long>(getAbs().toUSec() / 1000000L), static_cast<unsigned long>(getAbs().toUSec() % 1000000L));
}

template<typename T, typename D>
void TimeBase<T, D>::toString(char buf[StringBufSize]) const
{
	(void) snprintf(buf, StringBufSize, "%llu.%06lu", static_cast<unsigned long long>(toUSec() / 1000000UL), static_cast<unsigned long>(toUSec() % 1000000UL));
}
