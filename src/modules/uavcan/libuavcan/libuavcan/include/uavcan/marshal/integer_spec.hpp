...
public:
	enum
	{
		IsSigned = Signedness == SignednessSigned
	};
	enum
	{
		BitLen = BitLen_
...
public:
	...
	{
		return IsSigned ? StorageType(-max() - 1) : 0;
	}
	static UnsignedStorageType mask()
	{
		return Limits::mask();
...
template<CastMode CastMode>
class IntegerSpec<1, SignednessSigned, CastMode> ;
// Invalid instantiation

template<Signedness Signedness, CastMode CastMode>
class IntegerSpec<0, Signedness, CastMode> ;
// Invalid instantiation
...
public:
	...
	static void stream(Stream& s, const StorageType value, int)
	{
		...
		typedef typename Select<(sizeof(StorageType) >= sizeof(int)), StorageType, typename Select<RawType::IsSigned, int, unsigned>::Result>::Result TempType;
		s << TempType(value);
	}
};
