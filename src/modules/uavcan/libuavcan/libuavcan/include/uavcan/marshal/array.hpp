...
struct SquareMatrixTraits
{
	...
	enum
	{
		NumElementsInTriangle = ((1 + NumRowsCols) * NumRowsCols) / 2
	};

	static inline bool isIndexOnDiagonal(unsigned index)
	{
		return (index / NumRowsCols) == (index % NumRowsCols);
	}
	
	static inline int computeElementIndexAtRowCol(int row, int col)
	{
		return row * NumRowsCols + col;
	}
};
	...
	bool isSymmetric() const
	{
		for (int i = 0; i < Traits::NumRowsCols; ++i)
		{
			for (int k = 0; k < Traits::NumRowsCols; ++k)
			{
				...
				if ((i != k) && !areClose(*accessElementAtRowCol(i, k), *accessElementAtRowCol(k, i)))
				{
					return false;
				}
			}
...
public:
	...
	typedef typename StorageType<IntegerSpec<IntegerBitLen<EnumMax<Size, 2>::Result>::Result, SignednessUnsigned, CastModeSaturate> >::Type SizeType;
...
public:
	typedef typename StorageType<IntegerSpec<IntegerBitLen<EnumMax<MaxSize, 2>::Result>::Result, SignednessUnsigned, CastModeSaturate> >::Type SizeType;
...
public:
	enum
	{
		SizeBitLen = RawEncodedSizeType::BitLen
	};
...
class UAVCAN_EXPORT ArrayImpl: public Select<ArrayMode == ArrayModeDynamic, DynamicArrayBase<MaxSize>, StaticArrayBase<MaxSize> >::Result
{
	typedef ArrayImpl<T, ArrayMode, MaxSize> SelfType;
	typedef typename Select<ArrayMode == ArrayModeDynamic, DynamicArrayBase<MaxSize>, StaticArrayBase<MaxSize> >::Result Base;

public:
	enum
	{
		/// True if the array contents can be interpreted as a 8-bit string (ASCII or UTF8).
		IsStringLike = IsIntegerSpec<T>::Result && (T::MaxBitLen == 8 || T::MaxBitLen == 7) && (ArrayMode == ArrayModeDynamic)
	};
...
public:
	...
	{
		return BitSet<MaxSize>::operator[](ArrayBase::validateRange(pos));
	}
...
class UAVCAN_EXPORT Array: public ArrayImpl<T, ArrayMode, MaxSize_>
{
	...
	int encodeImpl(ScalarCodec& codec, const TailArrayOptimizationMode tao_mode, TrueType) const   /// Dynamic
	{
		...
		if (!self_tao_enabled)
		{
			const int res_sz = Base::RawEncodedSizeType::encode(typename StorageType<typename Base::RawEncodedSizeType>::Type(size()), codec, TailArrayOptDisabled);
			if (res_sz <= 0)
			{
				return res_sz;
			}
...
public:
	...
	enum
	{
		IsDynamic = ArrayMode == ArrayModeDynamic
	};
	enum
	{
		MaxSize = MaxSize_
	...
	enum
	{
		MinBitLen = (IsDynamic == 0) ? (static_cast<unsigned>(RawValueType::MinBitLen) * static_cast<unsigned>(MaxSize)) : 0
	};
	enum
	{
		MaxBitLen = static_cast<unsigned>(Base::SizeBitLen) + static_cast<unsigned>(RawValueType::MaxBitLen) * static_cast<unsigned>(MaxSize)
	};
	 ...
	 */
	template<typename R>
	typename EnableIf<sizeof((reinterpret_cast<const R*>(0))->size()) && sizeof((*(reinterpret_cast<const R*>(0)))[0]), bool>::Type operator==(const R& rhs) const
	{
		if (size() != rhs.size())
		{
			return false;
	 ...
	 */
	template<typename R>
	typename EnableIf<sizeof((reinterpret_cast<const R*>(0))->size()) && sizeof((*(reinterpret_cast<const R*>(0)))[0]), bool>::Type isClose(const R& rhs) const
	{
		if (size() != rhs.size())
		{
			return false;
	 ...
	 * @ref operator==()
	 */
	template<typename R> bool operator!=(const R& rhs) const
	{
		return !operator==(rhs);
	}
	...
	void appendFormatted(const char* const format, const A value)
	{
		...
		StaticAssert<(sizeof(A) <= sizeof(long double)) || (sizeof(A) <= sizeof(long long))>::check(); // Another stupid check to catch non-primitive types
	 ...
	 */
	template<typename R>
	typename EnableIf<sizeof((reinterpret_cast<const R*>(0))->begin()) && sizeof((reinterpret_cast<const R*>(0))->size())>::Type packSquareMatrix(const R& src_row_major)
	{
	 ...
	 */
	template<typename R>
	typename EnableIf<sizeof((reinterpret_cast<const R*>(0))->begin()) && sizeof((reinterpret_cast<const R*>(0))->size())>::Type unpackSquareMatrix(R& dst_row_major) const
	{
...
UAVCAN_EXPORT
inline typename EnableIf<!IsSameType<R, Array<T, ArrayMode, MaxSize> >::Result, bool>::Type operator==(const R& rhs, const Array<T, ArrayMode, MaxSize>& lhs)
{
