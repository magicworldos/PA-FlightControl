...
struct HasIsCloseMethod
{
	...
	enum
	{
		Result = sizeof(test<This, Rhs>(UAVCAN_NULLPTR))
	};
};
...
{
	return are_close_impl_::areCloseImplSecond(left, right, IntToType<are_close_impl_::HasIsCloseMethod<R, L>::Result>());
}
...
{
	return are_close_impl_::areCloseImplFirst(left, right, IntToType<are_close_impl_::HasIsCloseMethod<L, R>::Result>());
}
...
{
	return areFloatsClose(left, right, NumericTraits<float>::epsilon(), NumericTraits<float>::epsilon() * FloatComparisonEpsilonMult);
}
...
{
	return areFloatsClose(left, right, NumericTraits<double>::epsilon(), NumericTraits<double>::epsilon() * FloatComparisonEpsilonMult);
}
...
{
	return areFloatsClose(left, right, NumericTraits<long double>::epsilon(), NumericTraits<long double>::epsilon() * FloatComparisonEpsilonMult);
}
