...
public:
	...
	{
		return max_write_pos_;
	}
	void setMaxWritePos(uint16_t value)
	{
		max_write_pos_ = value;
	}
};
...
public:
	...
	{
		return TransferType(transfer_type_);
	}
 ...
 */
class UAVCAN_EXPORT TransferBufferManagerEntry: public ITransferBuffer, public LinkedListNode<TransferBufferManagerEntry>
{
	struct Block: LinkedListNode<Block>
	{
		enum
		{
			Size = MemPoolBlockSize - sizeof(LinkedListNode<Block> )
		};
...
public:
	...
	{
		return bufmgr_.access(key_);
	}
	ITransferBuffer* create()
	{
		return bufmgr_.create(key_);
	}
	void remove()
	{
		bufmgr_.remove(key_);
