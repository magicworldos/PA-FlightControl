...
public:
	...
	{
		return MonotonicDuration::fromMSec(MinTransferIntervalMSec);
	}
	static MonotonicDuration getMaxTransferInterval()
	{
		return MonotonicDuration::fromMSec(MaxTransferIntervalMSec);
	}
	
private:
	enum
	{
		IfaceIndexNotSet = MaxCanIfaces
	};
	...
	{
		return iface_index_ != IfaceIndexNotSet;
	}
	
	bool isMidTransfer() const
	{
		return buffer_write_pos_ > 0;
	}
