...
struct TimerCallCounter
{
	...
	{
		events_a.push_back(ev);
	}
	void callB(const uavcan::TimerEvent& ev)
	{
		events_b.push_back(ev);
	}
	...
	Binder bindA()
	{
		return Binder(this, &TimerCallCounter::callA);
	}
	Binder bindB()
	{
		return Binder(this, &TimerCallCounter::callB);
	}
};
...
{	
	...
	uavcan::Timer tm(node, [&count](const uavcan::TimerEvent&)
			{	count++;});
