...
public:
	...
	Binder bind()
	{
		return Binder(this, &SubscriptionCollector::handler);
	}
};
...
struct SubscriberWithCollector
{
	...
	int start()
	{
		return subscriber.start(collector.bind());
	}
};
...
public:
	...
	public:
		...
		typename ServiceCallResult::Status getStatus() const
		{
			return status_;
		}
		...
		typename DataType::Response& getResponse()
		{
			return response_;
		}
	};
...
public:
	...
	Binder bind()
	{
		return Binder(this, &ServiceCallResultCollector::handler);
	}
};
...
static inline void emulateSingleFrameBroadcastTransfer(CanDriver& can, uavcan::NodeID node_id, const MessageType& message, uavcan::TransferID tid)
{
	...
	uavcan::Frame frame(MessageType::DefaultDataTypeID, uavcan::TransferTypeMessageBroadcast, node_id, uavcan::NodeID::Broadcast, tid);
