...
struct RxFrameGenerator
{
	...
	{
		const uavcan::NodeID dst_nid = (bufmgr_key.getTransferType() == uavcan::TransferTypeMessageBroadcast) ? uavcan::NodeID::Broadcast : TARGET_NODE_ID;
		
		uavcan::Frame frame(data_type_id, bufmgr_key.getTransferType(), bufmgr_key.getNodeID(), dst_nid, transfer_id);
		...
		EXPECT_EQ(data.length(), frame.setPayload(reinterpret_cast<const uint8_t*>(data.c_str()), unsigned(data.length())));
		
		uavcan::RxFrame output(frame, uavcan::MonotonicTime::fromUSec(ts_monotonic), uavcan::UtcTime::fromUSec(ts_utc), iface_index);
...
static bool matchBufferContent(const uavcan::ITransferBuffer* tbb, const std::string& content)
{
	...
	std::cout << "Buffer content mismatch:" << "\n\tExpected: " << content << "\n\tActually: " << reinterpret_cast<const char*>(data) << std::endl;
	return false;
}
...
TEST(TransferReceiver, Basic)
{
     ...
     */
    CHECK_NOT_COMPLETE(rcv.addFrame(gen(0, "Foo", SET110, 0, 0), bk));ASSERT_EQ(TransferReceiver::getDefaultTransferInterval(), rcv.getInterval());
    ASSERT_EQ(0, rcv.getLastTransferTimestampMonotonic().toUSec());
     ...
     */
    CHECK_NOT_COMPLETE(rcv.addFrame(gen(0, "\x34\x12" "34567", SET100, 0, 100), bk));CHECK_COMPLETE(rcv.addFrame(gen(0, "foo", SET011, 0, 200), bk));
...
CHECK_NOT_COMPLETE(rcv.addFrame(gen(0, "\x9a\x78" "34567", SET100, 1, 1000), bk));CHECK_NOT_COMPLETE(rcv.addFrame(gen(0, "qwertyu", SET100, 1, 1100), bk));// Old toggle
CHECK_NOT_COMPLETE(rcv.addFrame(gen(0, "qwertyu", SET000, 1, 1100), bk));// Old toggle
CHECK_NOT_COMPLETE(rcv.addFrame(gen(0, "abcdefg", SET001, 1, 1200), bk));CHECK_NOT_COMPLETE(rcv.addFrame(gen(0, "4567891", SET001, 2, 1300), bk));// Next TID, but not SOT
...
ASSERT_FALSE(bufmgr.access(gen.bufmgr_key));          // Buffer must be removed
    ...
    CHECK_SINGLE_FRAME(rcv.addFrame(gen(0, "", SET110, 3, 2500), bk));ASSERT_EQ(2500, rcv.getLastTransferTimestampMonotonic().toUSec());

    CHECK_SINGLE_FRAME(rcv.addFrame(gen(0, "", SET110, 0, 3000), bk));CHECK_SINGLE_FRAME(rcv.addFrame(gen(0, "", SET110, 1, 3100), bk));CHECK_SINGLE_FRAME(rcv.addFrame(gen(0, "", SET110, 3, 3200), bk));CHECK_NOT_COMPLETE(rcv.addFrame(gen(1, "", SET110, 0, 3300), bk));// Old TID, wrong iface
...
CHECK_SINGLE_FRAME(rcv.addFrame(gen(0, "", SET110, 4, 3600), bk));ASSERT_EQ(3600, rcv.getLastTransferTimestampMonotonic().toUSec());
...
CHECK_SINGLE_FRAME(rcv.addFrame(gen(1, "qwe", SET110, 7, 1500100), bk));ASSERT_EQ(1500100, rcv.getLastTransferTimestampMonotonic().toUSec());
...
ASSERT_EQ(100000000, rcv.getLastTransferTimestampMonotonic().toUSec());
    ...
    CHECK_SINGLE_FRAME(rcv.addFrame(gen(0, "qwe", SET110, 8, 100000100), bk));ASSERT_EQ(100000100, rcv.getLastTransferTimestampMonotonic().toUSec());
...
TEST(TransferReceiver, OutOfOrderFrames)
{
    ...
    CHECK_NOT_COMPLETE(rcv.addFrame(gen(1, "1234567", SET100, 7, 100000000), bk));CHECK_NOT_COMPLETE(rcv.addFrame(gen(1, "-------", SET000, 7, 100000100), bk));  // Out of order
CHECK_NOT_COMPLETE(rcv.addFrame(gen(1, "-------", SET010, 7, 100000200), bk));// Out of order
CHECK_NOT_COMPLETE(rcv.addFrame(gen(1, "qwertyu", SET001, 7, 100000300), bk));CHECK_NOT_COMPLETE(rcv.addFrame(gen(1, "-------", SET011, 7, 100000200), bk));// Out of order
...
TEST(TransferReceiver, IntervalMeasurement)
{
    ...
    {
        CHECK_NOT_COMPLETE(rcv.addFrame(gen(1, "1234567", SET100, tid.get(), timestamp), bk));CHECK_NOT_COMPLETE(rcv.addFrame(gen(1, "qwertyu", SET001, tid.get(), timestamp), bk));CHECK_COMPLETE(rcv.addFrame(gen(1, "abcd", SET010, tid.get(), timestamp), bk));
...
TEST(TransferReceiver, UtcTransferTimestamping)
{
     ...
     */
    CHECK_NOT_COMPLETE(rcv.addFrame(gen(1, "1234567", SET100, 0, 1, 0), bk));CHECK_NOT_COMPLETE(rcv.addFrame(gen(1, "qwertyu", SET001, 0, 2, 0), bk));CHECK_COMPLETE(rcv.addFrame(gen(1, "abcd", SET010, 0, 3, 0), bk));
...
ASSERT_EQ(10, rcv.getLastTransferTimestampMonotonic().toUSec());
     ...
     * Restart recovery
     */
    CHECK_NOT_COMPLETE(rcv.addFrame(gen(0, "1234567", SET100, 1, 100000000, 800000000), bk));CHECK_NOT_COMPLETE(rcv.addFrame(gen(0, "qwertyu", SET001, 1, 100000001, 300000000), bk));CHECK_COMPLETE(rcv.addFrame(gen(0, "abcd", SET010, 1, 100000002, 900000000), bk));
