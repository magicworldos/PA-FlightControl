...
struct Transfer
{
	...
	{
		return (ts_monotonic == rhs.ts_monotonic) && ((!ts_utc.isZero() && !rhs.ts_utc.isZero()) ? (ts_utc == rhs.ts_utc) : true) && (priority == rhs.priority) && (transfer_type == rhs.transfer_type) && (transfer_id == rhs.transfer_id) && (src_node_id == rhs.src_node_id) && ((dst_node_id.isValid() && rhs.dst_node_id.isValid()) ? (dst_node_id == rhs.dst_node_id) : true) && (data_type == rhs.data_type) && (payload == rhs.payload);
	}
	
	std::string toString() const
	{
		std::ostringstream os;
		os << "ts_m=" << ts_monotonic << " ts_utc=" << ts_utc << " prio=" << int(priority.get()) << " tt=" << int(transfer_type) << " tid=" << int(transfer_id.get()) << " snid=" << int(src_node_id.get()) << " dnid=" << int(dst_node_id.get()) << " dtid=" << int(data_type.getID().get()) << "\n\t'" << payload << "'";
		return os.str();
	}
};
...
public:
	...
	void handleIncomingTransfer(uavcan::IncomingTransfer& transfer)
	{
		...
		const bool anonymous = single_frame && transfer.getSrcNodeID().isBroadcast() && (transfer.getTransferType() == uavcan::TransferTypeMessageBroadcast);
	...
	{
		return static_cast<unsigned>(transfers_.size());
	}
	bool isEmpty() const
	{
...
std::vector<uavcan::RxFrame> serializeTransfer(const Transfer& transfer)
{
	...
	uavcan::Frame frm(transfer.data_type.getID(), transfer.transfer_type, transfer.src_node_id, transfer.dst_node_id, transfer.transfer_id);
...
public:
	...
	Transfer makeTransfer(uavcan::TransferPriority priority, uavcan::TransferType transfer_type, uint8_t source_node_id, const std::string& payload, const uavcan::DataTypeDescriptor& type, uavcan::NodeID dst_node_id_override = uavcan::NodeID())
	{
		...
		const uavcan::NodeID dst_node_id = (transfer_type == uavcan::TransferTypeMessageBroadcast) ? uavcan::NodeID::Broadcast : (dst_node_id_override.isValid() ? dst_node_id_override : dst_node_id_);
	...
	template<int SIZE> void send(const Transfer (&transfers)[SIZE])
	{
		send(transfers, SIZE);
	}
};
