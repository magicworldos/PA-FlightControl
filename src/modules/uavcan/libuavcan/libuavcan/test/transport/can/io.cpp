...
static bool rxFrameEquals(const uavcan::CanRxFrame& rxframe, const uavcan::CanFrame& frame, uint64_t timestamp_usec, int iface_index)
{
	...
	return (static_cast<const uavcan::CanFrame&>(rxframe) == frame) && (rxframe.ts_mono == uavcan::MonotonicTime::fromUSec(timestamp_usec)) && (rxframe.iface_index == iface_index);
}
...
{	
	...
	driver.ifaces.at(0).writeable = true;
	driver.ifaces.at(1).writeable = true;
	const uavcan::CanFrame rx_frames[] =
	{	makeCanFrame(123, "rx0", STD), makeCanFrame(321, "rx1", EXT)};
