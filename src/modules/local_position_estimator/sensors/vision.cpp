...
void BlockLocalPositionEstimator::visionInit()
{
	...
	if (_visionStats.getCount() > REQ_VISION_INIT_COUNT)
	{
		...
		if (!_map_ref.init_done && _sub_vision_pos.get().xy_global)
		{
			...
			mavlink_and_console_log_info(&mavlink_log_pub, "[lpe] global origin init (vision) : lat %6.2f lon %6.2f alt %5.1f m", double(_sub_vision_pos.get().ref_lat), double(_sub_vision_pos.get().ref_lon), double(_sub_vision_pos.get().ref_alt));
...
void BlockLocalPositionEstimator::visionCorrect()
{
	...
	if (getDelayPeriods(_vision_delay.get() > 0.0f ? _vision_delay.get() : vision_delay, &i_hist) < 0)
	{
		return;
	}
