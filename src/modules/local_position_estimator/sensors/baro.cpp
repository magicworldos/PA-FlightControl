...
void BlockLocalPositionEstimator::baroInit()
{
	...
	if (_baroStats.getCount() > REQ_BARO_INIT_COUNT)
	{
		...
		mavlink_and_console_log_info(&mavlink_log_pub, "[lpe] baro init %d m std %d cm", (int )_baroStats.getMean()(0), (int )(100 * _baroStats.getStdDev()(0)));
...
void BlockLocalPositionEstimator::baroCorrect()
{
	...
	// residual
	Matrix<float, n_y_baro, n_y_baro> S_I = inv<float, n_y_baro>((C * _P * C.transpose()) + R);
