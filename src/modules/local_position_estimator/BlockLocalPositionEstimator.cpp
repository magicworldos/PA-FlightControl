...
void BlockLocalPositionEstimator::update()
{
	...
	if ((_fusion.get() & FUSE_BARO) && _sub_sensor.updated())
	{
		...
		{
			uint64_t baro_timestamp = _sub_sensor.get().timestamp + _sub_sensor.get().baro_timestamp_relative;
	...
	{
		map_projection_init(&_map_ref, _init_origin_lat.get(), _init_origin_lon.get());
		
		// set timestamp when origin was set to current time
		_time_origin = _timeStamp;
		
		mavlink_and_console_log_info(&mavlink_log_pub, "[lpe] global origin init (parameter) : lat %6.2f lon %6.2f alt %5.1f m", double(_init_origin_lat.get()), double(_init_origin_lon.get()), double(_altOrigin));
	}
	...
	for (int i = 0; i < n_x; i++)
	{
		for (int j = 0; j <= i; j++)
		{
			...
			{
				mavlink_and_console_log_info(&mavlink_log_pub, "%sreinit P (%d, %d) not finite", msg_label, i, j);
				reinit_P = true;
			}
			...
			if (i == j)
			{
				...
				if (_P(i, i) <= 0)
				{
					mavlink_and_console_log_info(&mavlink_log_pub, "%sreinit P (%d, %d) negative", msg_label, i, j);
					reinit_P = true;
				}
...
void BlockLocalPositionEstimator::updateSSParams()
{
	...
	float pn_t_noise_density = _pn_t_noise_density.get() + (_t_max_grade.get() / 100.0f) * sqrtf(_x(X_vx) * _x(X_vx) + _x(X_vy) * _x(X_vy));
