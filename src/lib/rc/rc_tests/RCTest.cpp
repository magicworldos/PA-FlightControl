...
bool RCTest::dsmTest(const char *filepath, unsigned expected_chancount, unsigned expected_dropcount, unsigned chan0)
{
	...
	while (EOF != (ret = fscanf(fp, "%f,%x,,", &f, &x)))
	{
		...
		bool result = dsm_parse(f * 1e6f, &frame[0], len, rc_values, &num_values, &dsm_11_bit, &dsm_frame_drops, max_channels);
...
bool RCTest::sbus2Test()
{
	...
	while (EOF != (ret = fscanf(fp, "%f,%x,,", &f, &x)))
	{
		...
		bool result = sbus_parse(now, &frame[0], len, rc_values, &num_values, &sbus_failsafe, &sbus_frame_drop, &sbus_frame_drops, max_channels);
